# Functions - Strategy (Backtesting)

The `strategy.*` namespace for backtesting and order placement: `strategy()` declaration, `strategy.entry`, `strategy.exit`, `strategy.close`, `strategy.cancel`, plus position/risk/trade inspection helpers.
