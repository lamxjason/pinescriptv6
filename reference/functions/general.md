# Functions - General (Math, Strings, Inputs, Time, Misc)

Core utilities not tied to indicators or strategies:
- `math.*` math, `str.*` string manipulation, `input.*` user inputs
- `alert()`, `alertcondition()`, `log.*` notifications
- Type-casts: `bool()`, `int()`, `float()`, `string()`, `color()`
- Time/date: `year()`, `month()`, `dayofmonth()`, `hour()`, `minute()`, `second()`, `time()`, `time_close()`, `timestamp()`, `weekofyear()`, `dayofweek()`
- Namespaces: `color.*` (functions, not constants), `runtime.*`, `syminfo.*`, `ticker.*`, `timeframe.*`
- Script declarations: `indicator()`, `library()`
- Series helpers: `na()`, `nz()`, `fixnan()`, `max_bars_back()`
