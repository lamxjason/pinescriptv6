# Functions - Drawing & Visuals

All visual-output functions: `plot`, `plotshape`, `plotchar`, `plotbar`, `plotcandle`, `plotarrow`, `bgcolor`, `barcolor`, `fill`, `hline`, plus the drawing-object namespaces `box.*`, `line.*`, `linefill.*`, `label.*`, `polyline.*`, `table.*`, and `chart.point.*`.
